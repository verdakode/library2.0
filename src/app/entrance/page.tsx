...
const ShelfUnit = ({ shelf, position }: ShelfUnitProps) => {
  ...
  const sampleBooks = [
    {
      title: "Introduction to " + shelf.name,
      content: "Sample content about " + shelf.name.toLowerCase(),
    },
    ...
    {
      title: shelf.name + " Principles",
      content: "Core principles of " + shelf.name.toLowerCase(),
    },
    {
      title: "Modern " + shelf.name,
      content: "Contemporary approaches to " + shelf.name.toLowerCase(),
    },
  ...
  return (
    <div className={`bookshelf-unit ${isCenter ? "h-48" : "h-64"}`}>
      <div className="p-4">
        <h2
          className={`text-[#F4E4BC] font-semibold mb-2 ${isCenter ? "text-lg" : "text-xl"}`}
          data-dewey={shelf.id}
        >
          {shelf.name}
        </h2>
      ...
      {selectedBook !== null && (
        ...
        >
          <div
            className="bg-[#F5E6D3] p-8 max-w-2xl w-full max-h-[80vh] overflow-y-auto rounded-lg shadow-2xl"
            onClick={(e) => e.stopPropagation()}
          >
            <h2 className="text-[#2B1810] text-2xl font-bold mb-4">
              {sampleBooks[selectedBook].title}
            </h2>
            <p className="text-[#2B1810] text-lg leading-relaxed">
              {sampleBooks[selectedBook].content}
            </p>
            <button
  ...
  return (
    <div className="library-room">
      ...
      <div className="fixed top-8 left-0 right-0 text-center z-10">
        <h1 className="library-title text-5xl">Welcome to My Digital Library</h1>
      </div>
